...
var Exec = function () {
	if (cli.debug) console.error('debug:'.grey.inverse + ' new exec queue.');
	this.queue = [];
	this.fresh = true;
};
...
Exec.prototype.send = function (args, next) {
	...
	if (!this.running) {
		process.nextTick(function () {
			if (cli.debug) console.error('debug:'.grey.inverse + ' next tick. calling next.');
			that.fresh = false;
			that.next();
		});
		this.running = true;
	}
	if (cli.debug) console.error('debug:'.grey.inverse + ' send called.');
...
Exec.prototype.go = function (queueObj) {
	...
	if (queueObj) {
		if (cli.debug) console.error('debug:'.grey.inverse + ' go called. queue depth %s. "%s"', this.queue.length, queueObj[0].bold);
...
Exec.prototype.next = function () {
	if (cli.debug) console.error('debug:'.grey.inverse + ' next called.');
