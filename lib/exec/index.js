...
var Exec = function () {
	if (cli.debug) console.error('debug: new exec queue.');
	this.queue = [];
	this.fresh = true;
};
...
Exec.prototype.go = function (queueObj) {
	...
	if (queueObj) {
		if (cli.debug) console.error('debug: go called. queue depth %s. "%s"', this.queue.length, queueObj[0]);
...
Exec.prototype.next = function () {
	if (cli.debug) console.error('debug: next called. queue depth %s.', this.queue.length);
